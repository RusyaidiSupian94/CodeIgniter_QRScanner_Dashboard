...
class Pickupsmh extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pickup/pickupsmh/pickupsmh');
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginsmh($no_siri);
		$hadir = $this->pickup_model->checkhadirandesitsmh($no_siri);
		...
		if($login)
		{
			...
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirsmh($data,$no_siri);
			redirect('pickupsmh/success');///update kehadiran
		}
		else if ($hadir){
	...
	public function success(){
		$this->load->view('pickup/headerpickup');
		$this->load->view('pickup/pickupsmh/successpagesmh');
		$this->load->view('pickup/footerpickup');
	}
}
