...
class Pickupqrmmd extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('qrpickup/qrpickupmmd/pickupmmd');
		$this->load->view('qrpickup/qrpickupmmd/footerpickup');
	}
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginmmd($code);
		$hadir = $this->pickup_model->checkhadirandesitmmd($code);
		...
		if($login)
		{
			...
			//$code=$this->input->post('code');
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirmmd($data,$code);
...
// 			echo "<script type='text/javascript'>alert('$message');window.location = ('success') </script>";
			...
			echo "<script type='text/javascript'>
				...
				function () {
    				window.location = 'pickupqrmmd/success';
				});</script>";
			...
			echo "<script type='text/javascript'>
				...
				function () {
    				window.location = 'pickupqrmmd/index';
				});</script>";
		}
		// else if($login){
	...
	public function success(){
		$this->load->view('qrpickup/headerpickup');
		$this->load->view('qrpickup/qrpickupmmd/successpage');
		$this->load->view('qrpickup/qrpickupmmd/footerpickup');
	}
}
