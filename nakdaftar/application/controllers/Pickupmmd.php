...
class Pickupmmd extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pickup/pickupmmd/pickup');
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginmmd($no_siri);
		$hadir = $this->pickup_model->checkhadirandesitmmd($no_siri);
		...
		if($login)
		{
			...
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirmmd($data,$no_siri);
			redirect('pickupmmd/success');///update kehadiran
		}
		else if ($hadir){
	...
	public function success(){
		$this->load->view('pickup/headerpickup');
		$this->load->view('pickup/pickupmmd/successpage');
		$this->load->view('pickup/footerpickup');
	}
}
