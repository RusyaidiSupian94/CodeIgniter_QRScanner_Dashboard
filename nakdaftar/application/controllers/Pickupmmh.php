...
class Pickupmmh extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pickup/pickupmmh/pickup');
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginmmh($no_siri);
		$hadir = $this->pickup_model->checkhadirandesitmmh($no_siri);
		...
		if($login)
		{
			...
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirmmh($data,$no_siri);
			redirect('pickupmmh/success');///update kehadiran
		}
		else if ($hadir){
	...
	public function success(){
		$this->load->view('pickup/headerpickup');
		$this->load->view('pickup/pickupmmh/successpage');
		$this->load->view('pickup/footerpickup');
	}
}
