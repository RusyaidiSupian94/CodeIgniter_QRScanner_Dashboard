...
class Smh extends CI_Controller {
	...
	{	
		...
		$this->load->view('smh/header', $data);
		$this->load->view('smh/login');
		$this->load->view('smh/footer');
	}
	...
	public function login_validation()
	{
		...
		$login = $this->smh_model->can_login($nric);
		...
		if($login)
		{
			...
			if($this->session->userdata('income') == '1' || $this->session->userdata('income') == '2' || $this->session->userdata('income') == '3' || $this->session->userdata('income') == '4' || $this->session->userdata('income') == '5' || $this->session->userdata('income') == '6')
			{
				...
				redirect(base_url().'smh/pengesahan_peserta');
			}

			else
			{
				redirect(base_url(). 'smh/profil_peserta');
			}
		}
		...
		/*if($this->form_validation->run())
		{
			...
			if($this->smh_model->can_login($nric))
			{
				$session_data = array(
								'nric' => $nric,
								'role' => $role,
	...
	{
		$data['userdata'] = $this->smh_model->fetch_userdata($this->session->userdata('nric'));
		$data['title'] = 'Nak Daftar | Aplikasi Pendaftaran';

		if($this->session->userdata('nric') != '')
		{
			$this->load->view('smh/header', $data);
			$this->load->view('smh/profile', $data);
			$this->load->view('smh/footer');
		}
	...
	public function peserta_survey()
	{
		...
		$data['title'] = 'Nak Daftar | Aplikasi Pendaftaran';
		$data['userdata'] = $this->smh_model->fetch_userdata($this->session->userdata('nric'));
		$data['user_data'] = $this->smh_model->fetch_single_data($this->session->userdata('nric'));

		if($this->session->userdata('nric') != '')
		{
		$this->load->view('smh/header', $data);
		$this->load->view('smh/survey', $data);
		$this->load->view('smh/footer');
		}
	...
	public function peserta_survey_prefil()
	{
		...
		{
		$this->load->view('smh/header', $data);
		$this->load->view('smh/survey_prefil', $data);
		$this->load->view('smh/footer');
		}
	...
	public function peserta_survey_pengesahan()
	{
		...
		if($this->form_validation->run())
		{
			...
			if($this->smh_model->update_userdata($data,$nric))
			{
				$this->smh_model->update_userdata($data, $this->input->post('hidden_id'));
				$data['userdata'] = $this->smh_model->fetch_userdata($this->session->userdata('nric'));
				$data['user_data'] = $this->smh_model->fetch_single_data($this->session->userdata('nric'));

				$data["fetch_data"] = $this->smh_model->fetch_data();
				...
				$this->load->view('smh/header', $data);
				$this->load->view('smh/pengesahan', $data);
				$this->load->view('smh/footer');
			}
		...
		{
			//false
			redirect(base_url() . 'smh/peserta_survey');
		}
	}
