...
class Pickupsmf extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pickup/pickupsmf/pickupsmf');
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginsmf($no_siri);
		$hadir = $this->pickup_model->checkhadirandesitsmf($no_siri);
		...
		if($login)
		{
			...
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirsmf($data,$no_siri);
			redirect('pickupsmf/success');///update kehadiran
		}
		else if ($hadir){
	...
	public function success(){
		$this->load->view('pickup/headerpickup');
		$this->load->view('pickup/pickupsmf/successpagesmf');
		$this->load->view('pickup/footerpickup');
	}
}
