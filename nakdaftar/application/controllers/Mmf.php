...
class Mmf extends CI_Controller {
	...
	{	
		...
		$this->load->view('mmf/header', $data);
		$this->load->view('mmf/login');
		$this->load->view('mmf/footer');
	}
	...
	public function login_validation()
	{
		...
		$login = $this->mmf_model->can_login($nric);
		...
		if($login)
		{
			...
			redirect(base_url().'mmf/pengesahan_peserta');////if the income got a value
	...
	{
		$data['userdata'] = $this->mmf_model->fetch_userdata($this->session->userdata('nric'));
		$data['title'] = 'Nak Daftar | Aplikasi Pendaftaran';

		if($this->session->userdata('nric') != '')
		{
			$this->load->view('mmf/header', $data);
			$this->load->view('mmf/profile', $data);
			$this->load->view('mmf/footer');
		}
	...
	public function peserta_survey()
	{
		...
		$data['title'] = 'Nak Daftar | Aplikasi Pendaftaran';
		$data['userdata'] = $this->mmf_model->fetch_userdata($this->session->userdata('nric'));
		$data['user_data'] = $this->mmf_model->fetch_single_data($this->session->userdata('nric'));

		if($this->session->userdata('nric') != '')
		{
		$this->load->view('mmf/header', $data);
		$this->load->view('mmf/survey', $data);
		$this->load->view('mmf/footer');
		}
	...
	public function peserta_survey_prefil()
	{
		...
		{
		$this->load->view('mmf/header', $data);
		$this->load->view('mmf/survey_prefil', $data);
		$this->load->view('mmf/footer');
		}
	...
	public function peserta_survey_pengesahan()
	{
		...
		if($this->form_validation->run())
		{
			...
			if($this->mmf_model->update_userdata($data,$nric))
			{

				$this->mmf_model->update_userdata($data, $this->input->post('hidden_id'));
				$data['userdata'] = $this->mmf_model->fetch_userdata($this->session->userdata('nric'));
				//$enter = $this->mmf_model->fetch_userdata($this->session->userdata('nric'));
				$data['user_data'] = $this->mmf_model->fetch_single_data($this->session->userdata('nric'));

				$data["fetch_data"] = $this->mmf_model->fetch_data();
				...
				$this->load->view('mmf/header', $data);
				$this->load->view('mmf/pengesahan', $data);
				$this->load->view('mmf/footer');
				//redirect(base_url() . 'mmf/generateQR');
			}
		...
		{
			//false
			redirect(base_url() . 'mmf/peserta_survey');
		}
	}
