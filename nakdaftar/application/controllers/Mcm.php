...
class Mcm extends CI_Controller {
	...
	{	
		...
		$this->load->view('mcm/header', $data);
		$this->load->view('mcm/login');
		$this->load->view('mcm/footer');
	}
	...
	public function login_validation()
	{
		...
		$login = $this->mcm_model->can_login($nric);
		...
		if($login)
		{
			...
			redirect(base_url().'mcm/pengesahan_peserta');////if the income got a value
			...
			// else
			// {
			// 	redirect(base_url(). 'mcm/profil_peserta');
			// }
		}
	...
	// {
	// 	$data['userdata'] = $this->mcm_model->fetch_userdata($this->session->userdata('nric'));
	// 	$data['title'] = 'Nak Daftar | Aplikasi Pendaftaran';

	// 	if($this->session->userdata('nric') != '')
	// 	{
	// 		$this->load->view('mcm/header', $data);
	// 		$this->load->view('mcm/profile', $data);
	// 		$this->load->view('mcm/footer');
	// 	}
	...
	// 	$data['title'] = 'Nak Daftar | Aplikasi Pendaftaran';
	// 	$data['userdata'] = $this->mcm_model->fetch_userdata($this->session->userdata('nric'));
	// 	$data['user_data'] = $this->mcm_model->fetch_single_data($this->session->userdata('nric'));

	// 	if($this->session->userdata('nric') != '')
	// 	{
	// 	$this->load->view('mcm/header', $data);
	// 	$this->load->view('mcm/survey', $data);
	// 	$this->load->view('mcm/footer');
	// 	}
	...
	// 	{
	// 	$this->load->view('mcm/header', $data);
	// 	$this->load->view('mcm/survey_prefil', $data);
	// 	$this->load->view('mcm/footer');
	// 	}
	...
	// 		if($this->mcm_model->update_userdata($data,$nric))
	// 		{

	// 			$this->mcm_model->update_userdata($data, $this->input->post('hidden_id'));
	// 			$data['userdata'] = $this->mcm_model->fetch_userdata($this->session->userdata('nric'));
	// 			//$enter = $this->mcm_model->fetch_userdata($this->session->userdata('nric'));
	// 			$data['user_data'] = $this->mcm_model->fetch_single_data($this->session->userdata('nric'));

	// 			$data["fetch_data"] = $this->mcm_model->fetch_data();
	...
	// 			$this->load->view('mcm/header', $data);
	// 			$this->load->view('mcm/pengesahan', $data);
	// 			$this->load->view('mcm/footer');
	// 			//redirect(base_url() . 'mcm/generateQR');
	// 		}
	...
	// 	{
	// 		//false
	// 		redirect(base_url() . 'mcm/peserta_survey');
	// 	}
	// }
