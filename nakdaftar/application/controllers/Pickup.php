...
class Pickup extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pickup/pickupamd/pickupamd');
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_login($no_siri);
		$hadir = $this->pickup_model->checkhadirandesit($no_siri);
		...
		if($login)
		{
			...
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadir($data,$no_siri);
			redirect('pickup/success');///update kehadiran
		}
		else if ($hadir){
	...
	public function success(){
		$this->load->view('pickup/headerpickup');
		$this->load->view('pickup/pickupamd/successpage');
		$this->load->view('pickup/footerpickup');
	}
}
