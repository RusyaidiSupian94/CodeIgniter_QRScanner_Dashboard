...
class Pickupqr extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('qrpickup/qrpickupamd/pickupamd');
		$this->load->view('qrpickup/qrpickupamd/footerpickup');
	}
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_login($code);
		$hadir = $this->pickup_model->checkhadirandesit($code);
		...
		if($login)
		{
			...
			//$code=$this->input->post('code');
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadir($data,$code);
...
// 			echo "<script type='text/javascript'>alert('$message');window.location = ('success') </script>";
	...
	public function success(){
		$this->load->view('qrpickup/headerpickup');
		$this->load->view('qrpickup/qrpickupamd/successpage');
		$this->load->view('qrpickup/qrpickupamd/footerpickup');
	}
}
