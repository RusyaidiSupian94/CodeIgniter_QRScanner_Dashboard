...
class Pickupmmf extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('pickup/pickupmmf/pickup');
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginmmf($no_siri);
		$hadir = $this->pickup_model->checkhadirandesitmmf($no_siri);
		...
		if($login)
		{
			...
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirmmf($data,$no_siri);
			redirect('pickupmmf/success');///update kehadiran
		}
		else if ($hadir){
	...
	public function success(){
		$this->load->view('pickup/headerpickup');
		$this->load->view('pickup/pickupmmf/successpage');
		$this->load->view('pickup/footerpickup');
	}
}
