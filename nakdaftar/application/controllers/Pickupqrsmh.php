...
class Pickupqrsmh extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('qrpickup/qrpickupsmh/pickupsmh');
		$this->load->view('qrpickup/qrpickupsmh/footerpickup');
	}
	...
	public function validation(){
		...
		$login = $this->pickup_model->can_loginsmh($code);
		$hadir = $this->pickup_model->checkhadirandesitsmh($code);
		...
		if($login)
		{
			...
			//$code=$this->input->post('code');
			$data['kehadiran'] = '1';
			$query = $this->pickup_model->hadirsmh($data,$code);
...
// 			echo "<script type='text/javascript'>alert('$message');window.location = ('success') </script>";
	...
	public function success(){
		$this->load->view('qrpickup/headerpickup');
		$this->load->view('qrpickup/qrpickupsmh/successpage');
		$this->load->view('qrpickup/qrpickupsmh/footerpickup');
	}
}
