...
class Grocery_crud_model  extends CI_Model  {
    ...
    function db_table_exists($smf = null)
    {
    	return $this->db->table_exists($smf);
    }
    ...
    function get_list()
    {
    	...
    	if(!empty($this->relation))
    	{
    		foreach($this->relation as $relation)
    		{
    			...
    			if($this->field_exists($related_field_title))
    				$select .= ", `{$this->smf}`.$related_field_title AS '{$this->smf}.$related_field_title'";
    		}
    	}
    	...
    	$results = $this->db->get($this->smf)->result();

    	return $results;
    }

    public function get_row($smf = null)
    {
    	$smf = $smf === null ? $this->smf : $smf;

    	return $this->db->get($smf)->row();
    }

    public function set_primary_key($field_name, $smf = null)
    {
    	$smf = $smf === null ? $this->smf : $smf;

    	$this->primary_keys[$smf] = $field_name;
    }
    ...
    protected function relation_n_n_queries($select)
    {
    	...
    	foreach($this->relation_n_n as $relation_n_n)
    	{
    		...
    		$select .= ", (SELECT GROUP_CONCAT(DISTINCT $field) FROM $selection_table "
    			...
    			."WHERE $relation_table.$primary_key_alias_to_this_table = `{$this->smf}`.$this_table_primary_key GROUP BY $relation_table.$primary_key_alias_to_this_table) AS $field_name";
    	}
    ...
    function get_total_results()
    {
    	...
    	if(!empty($this->relation_n_n))
    	{
            ...
            $this->db->select($this->smf . '.' . $key);
        }
        
        return $this->db->get($this->smf)->num_rows();
    }

    function set_basic_table($smf = null)
    {
    	if( !($this->db->table_exists($smf)) )
    		return false;
    ...
    function get_edit_values($primary_key_value)
    {
    	...
    	$result = $this->db->get($this->smf)->row();
    	return $result;
    }
    ...
    function join_relation($field_name , $related_table , $related_field_title)
    {
		...
		if($related_primary_key !== false)
		{
			...
			$this->db->join( $related_table.' as '.$unique_name , "$unique_name.$related_primary_key = {$this->smf}.$field_name",'left');
    ...
    function get_field_types_basic_table()
    {
    	...
    	foreach($this->db->query("SHOW COLUMNS FROM `{$this->smf}`")->result() as $db_field_type)
    	{
    	...
    	$results = $this->db->field_data($this->smf);
    ...
    function get_field_types($smf)
    {
    	$results = $this->db->field_data($smf);
    ...
    {
    	$primary_key_field = $this->get_primary_key();
    	return $this->db->update($this->smf,$post_array, array( $primary_key_field => $primary_key_value));
    }

    function db_insert($post_array)
    {
    	$insert = $this->db->insert($this->smf,$post_array);
    	if($insert)
    	{
    ...
    function db_delete($primary_key_value)
    {
    	...
    	$this->db->limit(1);
    	$this->db->delete($this->smf,array( $primary_key_field => $primary_key_value));
    ...
    {
    	if( $this->db->update($this->smf,array($field_name => ''),array($field_name => $filename)) )
    	{
    		return true;
    	}
    	else
    ...
    function field_exists($field,$smf = null)
    {
    	if(empty($smf))
    	{
    		$smf = $this->smf;
    	}
    	return $this->db->field_exists($field,$smf);
    }

    function get_primary_key($smf = null)
    {
    	if($smf == null)
    	{
    		if(isset($this->primary_keys[$this->smf]))
    		{
    			return $this->primary_keys[$this->smf];
    		}
    	...
    	{
    		if(isset($this->primary_keys[$smf]))
    		{
    			return $this->primary_keys[$smf];
    		}

	    	$fields = $this->get_field_types($smf);
